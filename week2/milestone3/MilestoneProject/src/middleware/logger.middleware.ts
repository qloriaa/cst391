...
/*
* Add logs for ans API endpoint
* Pattern : [id][timestamp] method:url START processing
...
export default function logger (req: Request, res:Response, next:NextFunction) {
    ...
    // get timestamp
    const now = new Date();
    const timestamp = [now.getFullYear(), '-', now.getMonth(), '-', now.getDate(), ' ', now.getHours(), ':', now.getMinutes(), ':', now.getSeconds()].join('');
